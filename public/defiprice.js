...
document.addEventListener('price', (e) => {
    ...
    if ( !table.querySelector('.logowrap img') ) {
        ...
        logo.onload = () => {
            table.querySelector('.logowrap').appendChild(logo)         
        }
        logo.onerror = () => {
        ...
        logo.src = `/assets/avalanche-tokens/${e.detail.id}/logo.png`;    
    }            
    let previous_price = table.querySelector('.price').innerHTML;

    table.querySelector('.price').innerHTML = isNaN(e.detail.price) ? e.detail.price : e.detail.price > 1000 ? e.detail.price.toFixed(0) : e.detail.price > 100 ? e.detail.price.toFixed(1) : e.detail.price > 1 ? e.detail.price.toFixed(2) : e.detail.price < .0001 ? e.detail.price.toFixed(8) : e.detail.price.toFixed(4)
    table.querySelector('.symbol').innerHTML = e.detail.symbol;
    ...
    if ( previous_price ) {
        ...
        } else if ( parseFloat(previous_price) > e.detail.price) {
            table.querySelector('.price').classList.remove('up')
            table.querySelector('.price').classList.add('down')
        }               
    } else {
        table.querySelector('.price').classList.remove('up')
        table.querySelector('.price').classList.remove('down')
    }      
    document.querySelector('table').setAttribute('timestamp', new Date(e.detail.timestamp).toLocaleTimeString());    
})
