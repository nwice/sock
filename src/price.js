import { dex_avaxprice, pair_contains, pair_token, find_token, get_price, refuse, pairnick } from './util.js';
...
const dexprices = (dex_list) => {  
  ...
  return Promise.all(dexes.map(async (dex) => {
    ...
    console.log(`${dex.symbol.toLowerCase()} pairs length:`, dex.pairs.length)
